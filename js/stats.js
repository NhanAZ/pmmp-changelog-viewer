...
const Stats = {
	...
	renderReleaseTimelineChart: function (data) {
		if (data.length === 0) {
			document.getElementById('release-timeline-chart').parentNode.innerHTML = '<div class="alert alert-warning">Not enough release date information available.</div>';
			return;
		}
		...
		if (sortedKeys.length === 0) {
			document.getElementById('release-timeline-chart').parentNode.innerHTML = '<div class="alert alert-warning">Could not parse release dates from the changelog files. Timeline cannot be generated.</div>';
			return;
		}
		...
		new Chart(ctx, {
			...
			data: {
				...
				datasets: [
					{
						data: values,
						backgroundColor: ['rgba(54, 162, 235, 0.7)', 'rgba(255, 99, 132, 0.7)', 'rgba(255, 206, 86, 0.7)', 'rgba(75, 192, 192, 0.7)', 'rgba(153, 102, 255, 0.7)', 'rgba(255, 159, 64, 0.7)', 'rgba(201, 203, 207, 0.7)', 'rgba(59, 143, 77, 0.7)', 'rgba(139, 69, 19, 0.7)', 'rgba(204, 0, 204, 0.7)'],
						borderWidth: 1,
					},
				],
			},
		...
		new Chart(ctx, {
			...
			data: {
				...
				datasets: [
					{
						data: values,
						backgroundColor: ['rgba(75, 192, 192, 0.7)', 'rgba(54, 162, 235, 0.7)', 'rgba(255, 99, 132, 0.7)', 'rgba(255, 206, 86, 0.7)', 'rgba(153, 102, 255, 0.7)'],
						borderWidth: 1,
					},
				],
			},
